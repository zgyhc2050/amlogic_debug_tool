...
class AmlDebugSystemOperationUi(AmlDebugBaseUi):
    ...
    def signals_connect_slots(self):
        ...
        self.__m_amlUi.AmlSystemCloseAvb_pushButton.clicked.connect(self.__click_close_avb)
    ...
    def __closeAvbProc(self):
        AmlCommon.exe_sys_cmd('adb reboot bootloader', True, self.__m_amlUi.terminalLog)
        AmlCommon.exe_sys_cmd('fastboot flashing unlock_critical', True, self.__m_amlUi.terminalLog)
        AmlCommon.exe_sys_cmd('fastboot flashing unlock', True, self.__m_amlUi.terminalLog)
        AmlCommon.exe_sys_cmd('fastboot reboot', True, self.__m_amlUi.terminalLog)
        timeCntS = 40
        self.__m_amlUi.terminalLog('__closeAvbProc: flashing unlock reboot platform, please wait ' + str(timeCntS) + ' s...')
        while timeCntS > 0 and self.__m_stop_thread == False: 
            time.sleep(1)
            timeCntS -= 1
        AmlCommon.exe_sys_cmd('adb root', True, self.__m_amlUi.terminalLog)
        AmlCommon.exe_sys_cmd('adb disable-verity', True, self.__m_amlUi.terminalLog)
        AmlCommon.exe_sys_cmd('adb reboot', True, self.__m_amlUi.terminalLog)
        timeCntS = 30
        self.__m_amlUi.terminalLog('__closeAvbProc: disable-verity reboot platform, please wait ' + str(timeCntS) + ' s...')
        while timeCntS > 0 and self.__m_stop_thread == False: 
            time.sleep(1)
            timeCntS -= 1
